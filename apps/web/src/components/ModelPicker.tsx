  ...
  return (
    <div>
      <div className="picker" ref={boxRef}>
        <input
          className="input"
          placeholder={full ? `Up to ${MAX_MODELS} models per run` : 'Search models: gpt-5, claude, deepseek, llama…'}
          value={query}
          disabled={full}
          role="combobox"
          aria-expanded={open}
        ...
        {open && !full && (
          <div className="picker-list" id="model-list" role="listbox">
            ...
            {matches.length === 0 && <div className="empty small">No models match “{query}”.</div>}
            {matches.map((m, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="num small muted" style={{ textAlign: 'right' }}>
                    {perM(m.promptPerM)} / {perM(m.completionPerM)}
                  </span>
                  ...
                  <span className="small faint" style={{ textAlign: 'right' }}>
                    {[m.contextLength ? `${compact(m.contextLength)} ctx` : '', m.reasoning ? 'reasoning' : ''].filter(Boolean).join(' · ') || ' '}
                  </span>
                </button>
              );
            })}
      ...
      {value.length > 0 && (
        <div className="picked" aria-label="Selected models">
          {value.map((v, i) => {
            const info = catalog.find((m) => m.provider === v.provider && m.id === v.model);
            return (
              <div className="picked-row" key={`${keyOf(v)}-${i}`}>
                ...
                <span className="grow" style={{ minWidth: 0 }}>
                  ...
                  <span className="small faint">
                    {v.provider} · {perM(info?.promptPerM)} / {perM(info?.completionPerM)} per 1M
                  </span>
                </span>
                <select
