...
- **Settings**: server address and optional API key (kept in the device keychain), with **Test connection**, which reports reachability, database, model and key status separately. It also has the interface language (Urdu, the default, or English), the **reply language** (Auto, Urdu script, Roman Urdu such as *"Ap k 20 customers hain."*, or English), and sounds and vibration on or off. Connection errors offer an **Open settings** shortcut.

Urdu is first-class: right-to-left layout, Noto Nastaliq Urdu for Urdu text, and each message's script detected on its own, so mixed chats render correctly. Answers with grouped or time-series results draw bar or line charts.
...
```bash
pnpm typecheck && pnpm lint && pnpm test   # 52 unit + integration tests (Jest, mocked network)
```
...
13 Playwright scenarios run at phone size. They check answers against ground-truth values in the `Eval_Retail` fixture. They cover follow-ups, reload persistence, stop and retry, recovery from a wrong server address, Roman Urdu replies, dark mode, the Urdu default, real voice and photo questions, charts, and performance budgets.
