  ...
  return (
    <View style={StyleSheet.absoluteFill} accessibilityViewIsModal>
      <Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: p.scrim, opacity: x }]}>
        <Pressable style={StyleSheet.absoluteFill} onPress={onClose} accessibilityRole="button" accessibilityLabel="Close conversations" />
      </Animated.View>
      <Animated.View
        style={[
          styles.panel,
          {
            ...
            transform: [{ translateX: x.interpolate({ inputRange: [0, 1], outputRange: [-panelWidth, 0] }) }],
          },
        ]}
      >
        <Pressable
          ...
          onPress={onNewChat}
          style={({ pressed }) => [styles.row, pressed && { backgroundColor: p.sunken }]}
        >
          <Feather name="edit" size={18} color={p.text} />
          <Text style={[type.label, { color: p.text }]}>New chat</Text>
        </Pressable>

        <Text style={[type.meta, styles.section, { color: p.muted }]}>Recents</Text>
        <ScrollView style={styles.list} contentContainerStyle={{ paddingBottom: 8 }}>
          {chats.length === 0 && <Text style={[type.meta, styles.empty, { color: p.faint }]}>Your conversations will appear here.</Text>}
          {chats.map((c) =>
            confirming === c.id ? (
              <View key={c.id} style={[styles.row, { backgroundColor: p.dangerSoft }]}>
                <Text style={[type.label, { color: p.text, flex: 1 }]} numberOfLines={1}>
                  Delete this chat?
                </Text>
                <Pressable accessibilityRole="button" accessibilityLabel="Cancel delete" onPress={() => setConfirming(null)} hitSlop={8}>
                  <Text style={[type.label, { color: p.muted }]}>Cancel</Text>
                </Pressable>
                <Pressable accessibilityRole="button" accessibilityLabel="Confirm delete" onPress={() => onDelete(c.id)} hitSlop={8}>
                  <Text style={[type.label, { color: p.danger, fontFamily: fonts.sansSemibold }]}>Delete</Text>
                </Pressable>
              </View>
            ) : (
              <Pressable
              ...
              >
                <Text style={[type.label, { color: p.text, fontFamily: fonts.sans }]} numberOfLines={1}>
                  {c.title}
                </Text>
                <Text style={[type.meta, { color: p.faint }]}>{relativeDay(c.updatedAt)}</Text>
              </Pressable>
            ),
          )}
        </ScrollView>
        ...
        <Pressable
          ...
          onPress={onSettings}
          style={({ pressed }) => [styles.row, { borderTopColor: p.border, borderTopWidth: StyleSheet.hairlineWidth }, pressed && { backgroundColor: p.sunken }]}
        >
          <Feather name="settings" size={18} color={p.text} />
          <Text style={[type.label, { color: p.text }]}>Settings</Text>
        </Pressable>
      </Animated.View>
    </View>
  );
...
const styles = StyleSheet.create({
  panel: { position: 'absolute', top: 0, bottom: 0, left: 0, paddingHorizontal: 8 },
