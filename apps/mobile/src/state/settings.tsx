...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    let alive = true;
    void Promise.all([storage.get<string>(URL_KEY), secrets.get(API_KEY), storage.get<UiLanguage>(LANG_KEY)]).then(
      ([url, key, lang]) => {
        if (!alive) return;
  ...
  const value = useMemo(() => ({ ready, server, language, save, setLanguage }), [ready, server, language, save, setLanguage]);
